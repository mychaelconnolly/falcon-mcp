...
```bash
curl -o .env https://raw.githubusercontent.com/mychaelconnolly/falcon-mcp/main/.env.example
```
...
#FALCON_MCP_MODULES=workflow,dashboards,detections,hosts,intel
...
falcon-mcp --modules workflow,dashboards,detections,hosts,intel
```

```bash
# Environment variable (fallback)
export FALCON_MCP_MODULES=workflow,dashboards,detections,hosts,intel
falcon-mcp
```
